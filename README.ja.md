...
極大魔法はただ大きいだけではありません。別の魔法陣を外枠に敷き、本来の魔法陣を中心に縮めて据え、
外枠の帯の上に小さな魔法陣をいくつも配って、外から順に展開します。

格もハッシュから決まるので、同じコマンドはいつも同じ格です（`cargo run` は極大魔法）。大魔法と極大魔法を引いたときだけ、
魔法陣の下に `✦ 極大魔法` のように名乗ります。
